...
cate_col <- c('challengeID', names(level_col[level_col>1 & level_col<4]))
...
f1 <- as.formula(paste("gpa ~ -1+", paste(names(cate_df)[8:ncol(cate_df)],collapse="+")))
tmp <- model.matrix(f1, cate_df)
fit <- glmnet(tmp, cate_df$gpa[as.numeric(row.names(tmp))], intercept = F)
cvfit <- cv.glmnet(tmp, cate_df$gpa[as.numeric(row.names(tmp))])
coef_tmp <- coef(cvfit, s=0.12)#s='lambda.1se')
print(coef_tmp[coef_tmp!=0])
